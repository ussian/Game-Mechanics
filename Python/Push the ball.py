...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self, width, height, color):
        ...
        self.height = height
        self.color = color
        self.image = pygame.Surface((width, height)) #size of image
        self.image.fill(color) # fill with the color
    ...
    def mirror_border(self): # draws a rectangle with the height/width of the player on the oposite part of the screen
        if self.rect.top < 0:
            pygame.draw.rect(screen, (self.color), pygame.Rect((self.rect.x,HEIGHT), (self.width,self.rect.y)))

        if self.rect.bottom > HEIGHT:
            pygame.draw.rect(screen, (self.color), pygame.Rect((self.rect.x,0), (self.width,self.rect.y - HEIGHT + self.height)))

        if self.rect.right > WIDTH:
            pygame.draw.rect(screen, (self.color), pygame.Rect((0, self.rect.y), (self.rect.x - WIDTH + self.width,self.width)))

        if self.rect.left < 0:
            pygame.draw.rect(screen, (self.color), pygame.Rect((WIDTH, self.rect.y), (self.rect.x, self.width)))
...
player = Player(50, 50, GREEN)
all_sprites.add(player)

ball = Ball(10, 10, YELLOW)
